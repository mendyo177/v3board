...
class ServerService
{
    ...
    public function getAvailableV2node(User $user)
    {
        ...
        foreach ($v2node as $key => $v) {
            ...
            // 自定义节点（raw_uri）强制显示在线
            try {
                $ns = $v2node[$key]['network_settings'] ?? [];
                if (is_array($ns) && !empty($ns['raw_uri'])) {
                    $v2node[$key]['last_check_at'] = time();
                }
    ...
    private function mergeData(&$servers)
    {
        foreach ($servers as $k => $v) {
            ...
            // 自定义节点（raw_uri）后台列表强制在线展示
            try {
                if (($v['type'] ?? '') === 'v2node') {
                    $ns = $v['network_settings'] ?? [];
                    if (is_array($ns) && !empty($ns['raw_uri'])) {
                        $servers[$k]['last_check_at'] = time();
                        $servers[$k]['last_push_at'] = time();
                    }
                }
            } catch (\Throwable $e) { /* ignore */ }
